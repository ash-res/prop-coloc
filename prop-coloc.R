...
# figs (optional): return plots of the fitted multivariable variant--trait associations of top 10 variants for each trait
# traits (optional): a character vector of the two trait names

# outputs
# full: p-value of prop-coloc-full test based on top 10 variants for each trait
# eta_full: eta (proportionality constant) estimate based on top 10 variants for each trait
# LM_full: p-value of LM test based on top 10 variants for each trait 
...
prop.coloc <- function(b1,se1,b2,se2,n,ld,tau=NULL,alpha=NULL,prune=NULL,J=NULL,figs=NULL,traits=NULL){
  ...
  # calculate p-values and re-order traits
  p1 <- 2*(1-pnorm(abs(b1)/se1))
  p2 <- 2*(1-pnorm(abs(b2)/se2))
  t2 <- (1*ifelse(sort(p1)[1]<sort(p2)[1],1,0))+(2*(1-ifelse(sort(p1)[1]<sort(p2)[1],1,0))) # trait 2 has the strongest lead variant
  t1 <- (1*ifelse(sort(p1)[1]>sort(p2)[1],1,0))+(2*(1-ifelse(sort(p1)[1]>sort(p2)[1],1,0)))
  if(t1==2){B1=b2; B2=b1; Se1=se2; Se2=se1; P1=p2; P2=p1}
  if(t1==2){b1=B1; b2=B2; se1=Se1; se2=Se2; p1=P1; p2=P2}
  if(t1==2){rm(B1,B2,Se1,Se2,P1,P2)}
  
  # pruning variants
  i=1
  colnames(ld) <- as.character(1:nrow(ld)); rownames(ld) <- as.character(1:nrow(ld))
  while(i<=(length(b1)*2)){
    if((i %% 2) != 0){top <- order(p1)[((i+1)/2)]}
    if((i %% 2) == 0){top <- order(p2)[(i/2)]}
    del <- names(which((ld[top,-top]^2)>prune))
     if(length(del)>0){
      ...
      p1 <- p1[-which(colnames(ld)%in%del)]; p2 <- p2[-which(colnames(ld)%in%del)]
  ...
  sel <- unique(c(order(p1)[1:J1],order(p2)[1:J1]))
  sel <- sort(sel)
  b1 <- b1[sel]; se1 <- se1[sel]; b2 <- b2[sel]; se2 <- se2[sel]; ld <- ld[sel,sel]; p1 <- p1[sel]; p2 <- p2[sel]; J <- length(b1)
  ...
  if(length(alpha)==1){res <- cond.test(alpha); cond.res=res[1]; conv <- res[2]
    res.list <- list("full"=pval_full, "eta_full"=eta_full, "LM_full"=LM_full, "cond"=cond.res, "eta"=eta.naive, "LM"=LM.0, "convergence"=conv)
  }
  
  if(length(alpha)>1){
    res <- sapply(alpha,cond.test); cond.res=res[1,]; conv <- res[2,]
      if(sum(cond.res)>0){
        res.list <- list("full"=pval_full, "eta_full"=eta_full, "LM_full"=LM_full, "cond"=alpha[which(cond.res)[1]], "eta"=eta.naive, "LM"=LM.0, "convergence"=conv[which(cond.res)[1]])
      }
      if(sum(cond.res)==0 & sum(conv)==length(alpha)){
        res.list <- list("full"=pval_full, "eta_full"=eta_full, "LM_full"=LM_full, "cond"=max(alpha), "eta"=eta.naive, "LM"=LM.0, "convergence"=TRUE)
      }
      if(sum(cond.res)==0 & sum(conv)!=length(alpha)){
        res.list <- list("full"=pval_full, "eta_full"=eta_full, "LM_full"=LM_full, "cond"=FALSE, "eta"=eta.naive, "LM"=LM.0, "convergence"=FALSE)
      }
  }
  return(res.list)
}